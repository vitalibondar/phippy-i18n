...
| Goldie | Ґолді | `Ґолді` is the spelling chosen for this character. § 122(3) of the orthography permits both `г` and `ґ` when rendering /g/ in foreign personal names, so neither is required. Indeclinable feminine, same class as Фіппі. No existing precedent found. |
